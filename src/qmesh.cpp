...
QMesh *QMesh::CreateWithCircle(float radius, QVector relativePosition){
	QMesh * res=new QMesh();
	res->position=relativePosition;
...
QMesh::MeshData QMesh::GenerateRectangleMeshData(QVector size,QVector centerPosition,QVector grid, float particleRadius)
{
	...
	if(grid.x<=1 && grid.y<=1){
		...
		res.springList.push_back(pair(0,1));
		res.springList.push_back(pair(1,2));
		res.springList.push_back(pair(2,3));
		res.springList.push_back(pair(3,0));

		res.internalSpringList.push_back(pair(0,2));
		res.internalSpringList.push_back(pair(1,3));
	}else{
