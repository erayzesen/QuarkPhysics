...
void QCollision::PolylineAndPolygon(vector<QParticle*> &polylineParticles, vector<QParticle*> &polygonParticles, vector<Contact> &contacts)
{
   ...
   */
	...
	float maxRayLength=numeric_limits<float>::max()*0.1f;
	for(int i=0;i<polygonParticles.size();i++){
		...
		QVector bisectorUnit=(np->GetGlobalPosition()-pp->GetGlobalPosition()).Normalized().Perpendicular();
...
void QCollision::PolygonAndPolygon(vector<QParticle*> &particlesA,vector<QParticle*> &particlesB, vector<QCollision::Contact> &contacts,QWorld *world){
	//The algorithm is an implement of the Separating Axis Theorem(SAT).
	/*
